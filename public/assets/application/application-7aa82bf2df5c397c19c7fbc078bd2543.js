...
(function() {
  ...
  DateSelectSmoke = (function() {
    function DateSelectSmoke() {
      this.select = $('.date-select.smoke');
      this.dates = $('.date-select.smoke a');
      this.date_in_text = $('span.date_in_text_smoke');
        ...
        return function(e) {
          ...
          return $.ajax('/find-smokes?date=' + date_selected, {
            ...
            success: function(data, textStatus, jqXHR) {
              var date_in_view, rangeValue, rangeValuePercentage, rangeWidth;
              date_in_view = new Date(data['date']);
              date_in_view.locale = "nl";
              _this.date_in_text.text(date_in_view.strftime('%d %B'));
...
(function() {
  ...
  DateSelectSport = (function() {
    function DateSelectSport() {
      this.select = $('.date-select.sport');
      this.dates = $('.date-select.sport a');
      this.date_in_text = $('span.date_in_text_sport');
      Date.ext.locales['nl'] = {
        B: ['januari', 'februari', 'maart', 'april', 'mei', 'juni', 'juli', 'augustus', 'september', 'oktober', 'november', 'december'],
        x: '%d.%m.%Y'
      };
        ...
        return function(e) {
          ...
          return $.ajax('/find-sport?date=' + date_selected, {
            ...
            success: function(data, textStatus, jqXHR) {
              ...
              date_in_view = new Date(data['date']);
              date_in_view.locale = "nl";
              return _this.date_in_text.text(date_in_view.strftime('%d %B'));
            }
          });
        };
      })(this));
  ...
  $(function() {
    var date_select2;
    if (($('.date-select')).length > 0) {
      return date_select2 = new DateSelectSport;
    }
  });
  ...
  Graph = (function() {
    ...
    Graph.prototype.initializeData = function(data) {
      ...
      for (_i = 0, _len = data.length; _i < _len; _i++) {
        row = data[_i];
        new_data.push({
          'm': new Date(row['m']).strftime('%Y-%m-%d'),
          "a": row["a"]
        });
      }
      return new_data;
...
(function() {
  $(function() {
    var client, messageCount, user_hash;
    messageCount = 0;
    client = new Faye.Client('/faye');
    user_hash = ($('.userhash')).text();
    client.subscribe('/message/' + user_hash, function(payload) {
      var messageCenter;
      console.log(payload);
  ...
  SubmitForm = (function() {
    function SubmitForm() {
      ...
      form.on('change', (function(_this) {
        return function(e) {
          var date, date_input, date_parsed, submit_form, year;
